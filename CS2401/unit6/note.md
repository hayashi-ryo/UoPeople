...
今週のユニットでは、開発を行うソフトウェアの特性を計測するためにアイデアについて学習を行った。特に、ソフトウェアの複雑度を計測する指標であるサイクロマティック複雑度と凝集性を順序づけする方法は非常に重要であると考えるため、私の理解を整理していく。
一つ目：サイクロマティック複雑度
  サイクロマティック複雑度は、実行するコードがどの程度複雑であるかを示す指標の一つである。処理や分岐、ループなどをそれぞれ一つの分岐と考え、ソフトウェアの中でどの程度利用されているかを計測する。この値が高いほど保守者によるメンテナンスが難しく、保守性の低い特性を持つと考えることができる。今までの経験においても、if文やforループをネスト実装しているようなプログラムは理解が難しい経験があるため、今までの経験則とも近しいものを感じた。
二つ目：凝集性の順序づけ
  凝集性の順序づけは一つのソフトウェアの中にどの程度機能を設けるか、また機能がどの程度関連づけされているかを計測するために指標だ。今回学習した中で重大な気づきであったのは、一見凝集性の高いプログラムであっても、内容が全然関連づいておらず保守性の低いコードになってしまっている場合があるということである。このようなプログラムを「Coincidental cohesion」や「Logical cohesion」と呼ぶ場合がある。(Marsic, 2012)
  今後ソフトウェアの開発を進める上では、凝集性とカップリングのトレードオフと守りながら、保守性の高いコードを実装できるよう心掛けてゆきたい。
